...
# File parsing
pathF <- "/Volumes/MetaT_backup/project3/tempA_FWD"
pathR <- "/Volumes/MetaT_backup/project3/tempA_REV"
...
set.seed(100)
errF <- learnErrors(filtFs, nbases=1e8, multithread=TRUE) #100838975 total bases in 373805 reads from 5 samples will be used for learning the error rates.
errR <- learnErrors(filtRs, nbases=1e8, multithread=TRUE) #100917588 total bases in 373805 reads from 5 samples will be used for learning the error rates.
...
saveRDS(seqtab, "./seqtab_tempA.rds")
#to read it back to R: tempA <- readRDS("./seqtab_tempA.rds")
